/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   op.c                                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: kpolojar <kpolojar@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/04 22:53:19 by kpolojar          #+#    #+#             */
/*   Updated: 2022/11/07 20:55:58 by kpolojar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/op.h"
#include "../include/assembler.h"

// [index][0] = char *name
// [index][1] = int nb_of_args
// [index][2] = arg_types {T_REG, T_REG, T_REG}
// [index][3] = int statement code ( = index)
// [index][4] = int Cycles before execution
// [index][5] = char * (description?)
// [index][6] = bool ???
// [index][7] = bool ???

static t_op	g_op_tab[17] =
{
	{{0},	 	0, {0}, 													0,	0, 		{0},									0, 0},
	{"live", 	1, {T_DIR}, 												1, 	10, 	"alive", 								0, 0},
	{"ld", 		2, {T_DIR | T_IND, T_REG}, 									2, 	5, 		"load", 								1, 0},
	{"st", 		2, {T_REG, T_IND | T_REG}, 									3, 	5, 		"store", 								1, 0},
	{"add", 	3, {T_REG, T_REG, T_REG}, 									4, 	10, 	"addition", 							1, 0},
	{"sub", 	3, {T_REG, T_REG, T_REG}, 									5, 	10, 	"soustraction",							1, 0},
	{"and", 	3, {T_REG | T_DIR | T_IND, T_REG | T_IND | T_DIR, T_REG}, 	6, 	6, 		"et (and  r1, r2, r3   r1&r2 -> r3", 	1, 0},
	{"or", 		3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 	7, 	6, 		"ou  (or   r1, r2, r3   r1 | r2 -> r3",	1, 0},
	{"xor", 	3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 	8, 	6, 		"ou (xor  r1, r2, r3   r1^r2 -> r3", 	1, 0},
	{"zjmp", 	1, {T_DIR}, 												9, 	20, 	"jump if zero", 						0, 1},
	{"ldi", 	3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 			10, 25, 	"load index", 							1, 1},
	{"sti", 	3, {T_REG, T_REG | T_DIR | T_IND, T_DIR | T_REG}, 			11, 25, 	"store index", 							1, 1},
	{"fork", 	1, {T_DIR}, 												12, 800, 	"fork", 								0, 1},
	{"lld", 	2, {T_DIR | T_IND, T_REG},									13, 10, 	"long load", 							1, 0},
	{"lldi", 	3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 			14, 50, 	"long load index", 						1, 1},
	{"lfork", 	1, {T_DIR}, 												15, 1000,	"long fork", 							0, 1},
	{"aff", 	1, {T_REG}, 												16, 2, 		"aff", 									1, 0}
};

void	set_asm_op_tab(t_asm *a)
{
	int i;

	i = 0;
	while(i < 17)
	{
		a->op_tab[i] = g_op_tab[i];
		i++;
	}
}

int	get_id_by_name(char *name, t_asm *a)
{
	int i;

	i = 0;
	while (i < 17)
	{
		if (ft_strequ(name, a->op_tab[i].name))
			return (a->op_tab[i].op_code);
		i++;
	}
	exit_program(-1, "No such statement!");
	return (-1);
}
